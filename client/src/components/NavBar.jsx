    ...
    return (
      <nav className="bg-gray-800 p-1 mt-0 w-full fixed top-0 z-20">
        ...
        <div className="container mx-auto my-8 flex flex-wrap items-center justify-between">
          <div className="flex justify-end space-x-4 -mb-4 -ml-6">
            <ul className="flex items-center space-x-4">
              {items.map(({ name, path, onClick }, index) => {
                ...
                return (
                  <li key={index}>
                    <NavLink
                      className={`inline-block py-2 px-4 text-black no-underline rounded font-bodyFont ${
                        isActive ? "bg-designColor" : "hover:bg-designColor"
                      }`}
                      to={path}
                      onClick={onClick}
                    >
